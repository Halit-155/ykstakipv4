...
const String _apiKey = "AIzaSyDR2SXYCEX7VyIHLjcIVhzSDX3NpErAVDU";
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        useMaterial3: true,
        // Standart font kullanımı (Hata riskini sıfırlar)
        fontFamily: 'Roboto',
...
class VeriDeposu {
  ...
  static final Map<String, List<KonuDetay>> dersKonuAgirliklari = {
    ...
    "TYT Matematik": [
      KonuDetay("Temel Kavramlar", 4),
      KonuDetay("Problemler", 13),
      KonuDetay("Fonksiyonlar", 2),
      KonuDetay("Geometri", 10)
    ],
    "TYT Fizik": [
      KonuDetay("Fizik Bilimi", 1),
      KonuDetay("Madde ve Öz.", 2),
      KonuDetay("Hareket", 2),
      KonuDetay("Optik", 2)
    ],
    "TYT Kimya": [
    ...
    "TYT Coğrafya": [
      ...
      KonuDetay("Harita Bilgisi", 2)
    ],
    "TYT Felsefe": [KonuDetay("Felsefeye Giriş", 1)],
    "TYT Din": [KonuDetay("Bilgi ve İnanç", 1)],
    "AYT Matematik": [
    ...
    "AYT Fizik": [
      ...
      KonuDetay("Atışlar", 2),
      KonuDetay("Elektrik", 4)
    ],
    "AYT Kimya": [
      KonuDetay("Modern Atom", 2),
      KonuDetay("Gazlar", 3),
      KonuDetay("Sıvı Çözeltiler", 3),
      KonuDetay("Organik", 6)
    ],
    "AYT Biyoloji": [
      KonuDetay("Sinir Sistemi", 2),
      KonuDetay("Endokrin", 2),
      KonuDetay("Sistemler", 10),
      KonuDetay("Bitki", 5)
    ],
    "AYT Edebiyat": [
    ...
    "AYT Tarih-1": [KonuDetay("İlk Çağ", 2), KonuDetay("Türk Tarihi", 2)],
    "AYT Coğrafya-1": [KonuDetay("Biyoçeşitlilik", 2), KonuDetay("Ekonomi", 3)],
    "AYT Tarih-2": [KonuDetay("Osmanlı", 5), KonuDetay("İnkılap Tarihi", 4)],
    "AYT Coğrafya-2": [
      KonuDetay("Türkiye Ekonomisi", 3),
      KonuDetay("Bölgeler", 2)
    ],
    "AYT Felsefe Gr": [
    ...
    "AYT Din": [KonuDetay("İslam Düşüncesi", 2)]
  };
  ...
  static dynamic girisKontrol(String k, String s) {
    ...
    if (k == "ogretmen1" && s == "1234")
      return Ogretmen(id: "t1", ad: "Mehmet Hoca", brans: "Matematik");
    return null;
  }

  static void girisSayaciArtir(String id, bool isOgrenci) {
    /* Sayaç artırma mantığı */
  }
}
...
class GeminiServisi {
  static Future<String> generateText(String prompt) async {
    if (_apiKey.isEmpty || _apiKey == "BURAYA_API_KEY_YAZIN")
      return "Lütfen API Key giriniz.";
    try {
      final url = Uri.parse(
            ...
            "contents": [
              {
                "parts": [
                  {
                    "text":
                        "Sen uzman bir öğretmensin. Bu soruyu analiz et ve adım adım, anlaşılır şekilde çöz."
                  },
                  {
                    "inline_data": {
  ...
  static Future<List<Gorev>> programOlustur(String sinif, String alan,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        String hamVeri = jsonDecode(response.body)['candidates'][0]['content']
            ['parts'][0]['text'];

        // Markdown temizliği (```json ... ```)
        hamVeri =
            hamVeri.replaceAll("```json", "").replaceAll("```", "").trim();
        ...
        Navigator.pushReplacement(
            ...
            MaterialPageRoute(
                builder: (c) => const Scaffold(
                    body: Center(child: Text("Öğretmen Paneli")))));
    } else {
            ...
            child: GridView.count(
              ...
              children: [
                ...
                _buildMenuCard(
                    ...
                    Colors.amberAccent),
                _buildMenuCard(context, "Günlük Takip", Icons.today,
                    const GunlukTakipEkrani(), Colors.teal, Colors.greenAccent),
              ],
            ),
          ),
        ],
...
class _SUEState extends State<SoruUretecEkrani> {
  ...
  Future<void> _uret() async {
    ...
    String p =
        "$ders dersi $konu konusunda ${zorluk ?? 'orta'} seviye bir adet çoktan seçmeli YKS sorusu yaz. Şıkları ve cevabı da ver.";
...
class _YZSEState extends State<YapayZekaSohbetEkrani> {
  ...
  void _send() async {
    ...
    String r = await GeminiServisi.generateText(
        "Sen bir rehber öğretmenisin. Soru: $t");
...
class _TPEState extends State<TumProgramEkrani>
  ...
  void _edit(Gorev g) {
    String? d =
            VeriDeposu.dersKonuAgirliklari.containsKey(g.ders) ? g.ders : null,
        k = g.konu,
        a = g.aciklama;
    showDialog(
        context: context,
            ...
            builder: (c, st) => AlertDialog(
                    ...
                    content: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      if (d != null)
                        DropdownButton<String>(
                            isExpanded: true,
                            value: (VeriDeposu.dersKonuAgirliklari[d]!
                                    .any((x) => x.ad == k))
                                ? k
                                : null,
                            hint: const Text("Konu"),
                            items: VeriDeposu.dersKonuAgirliklari[d]!
  ...
  void _olustur() {
    ...
    if (secilen.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Lütfen en az bir ders seçin!")));
      return;
    }
    secilen.shuffle();
    ...
    int blok = toplamDk ~/ (dersSuresi + molaSuresi);
    if (blok < 1) blok = 1;
    ...
    for (int h = 1; h <= 1; h++) {
      ...
      for (var gun in gunler) {
        ...
        for (int i = 0; i < blok; i++) {
          String d = secilen[idx % secilen.length];
          idx++;
          int s = (basla.hour * 60 + basla.minute) +
              (i * (dersSuresi + molaSuresi));
          int e = s + dersSuresi;
          String ss =
              "${(s ~/ 60).toString().padLeft(2, '0')}:${(s % 60).toString().padLeft(2, '0')} - ${(e ~/ 60).toString().padLeft(2, '0')}:${(e % 60).toString().padLeft(2, '0')}";
          program.add(Gorev(
              hafta: h,
              gun: gun,
              saat: ss,
...
class _PSEState extends State<ProgramSihirbaziEkrani> {
  ...
  void _olusturAI() async {
    ...
    List<Gorev> program = await GeminiServisi.programOlustur(
    ...
    if (program.isNotEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("AI program oluşturamadı. Tekrar deneyin.")));
    }
  }
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 10),
              const Text(
                  "Bana hedeflerinden bahset, sana en uygun programı hazırlayayım!",
                  textAlign: TextAlign.center,
                  style: TextStyle(fontSize: 16)),
              const SizedBox(height: 20),
              DropdownButtonFormField(
                  value: sinif,
                  decoration: const InputDecoration(labelText: "Sınıfın"),
                  items: ["9", "10", "11", "12", "Mezun"]
              ...
              TextFormField(
                  decoration:
                      const InputDecoration(labelText: "Hedefin (Bölüm/Üni)"),
                  initialValue: hedef,
                  onSaved: (v) => hedef = v!),
              const SizedBox(height: 10),
              TextFormField(
                  decoration: const InputDecoration(
                      labelText: "En Zayıf Olduğun Dersler"),
                  initialValue: zayif,
                  onSaved: (v) => zayif = v!),
              const SizedBox(height: 10),
              TextFormField(
                  decoration: const InputDecoration(
                      labelText: "Günlük Kaç Saat Çalışabilirsin?"),
                  initialValue: saat,
...
class RozetlerEkrani extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(
        ...
        child: Scaffold(
            appBar: AppBar(
                title: const Text("Rozetler & Liderlik"),
            ...
            body: TabBarView(children: [
              ListView(children: [
                _buildSection("🏆 Seviye", v),
                _buildSection("🎯 Soru", s),
                _buildSection("📚 Konu", k),
                _buildSection("📝 Deneme", d)
              ]),
              ListView.builder(
                  itemCount: siraliOgrenciler.length,
                  itemBuilder: (c, i) {
                    var o = siraliOgrenciler[i];
                    return Card(
                        color: o.id == ogrenci.id
                            ? Colors.green[50]
                            : Colors.white,
                        child: ListTile(
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ...l.map((r) => ListTile(
          leading: CircleAvatar(
              backgroundColor:
                  r.kazanildi ? r.renk.withOpacity(0.2) : Colors.grey[200],
              child: Icon(r.ikon, color: r.kazanildi ? r.renk : Colors.grey)),
          title: Text(r.ad),
...
class _KTE extends State<KonuTakipEkrani> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: ListView(
            children: VeriDeposu.dersKonuAgirliklari.keys
                .map((d) => ExpansionTile(
                    title: Text(d),
                    children: VeriDeposu.dersKonuAgirliklari[d]!
                        .map((k) => CheckboxListTile(
                            title: Text(k.ad),
                            value:
...
class _SCEState extends State<SoruCozumEkrani> {
  ...
  Future<void> _coz() async {
    ...
    String c = await GeminiServisi.soruCoz(_image!);
    setState(() {
      _cozum = c;
      _loading = false;
    });
    ...
    return Scaffold(
        ...
        body: SingleChildScrollView(
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 10),
              ElevatedButton(
                  onPressed: _loading ? null : _coz,
                  child: _loading
                      ? const CircularProgressIndicator()
                      : const Text("ÇÖZ")),
              if (_cozum.isNotEmpty)
                Container(
